...
SYSTEM_PROMPT = """You are an expert quantitative portfolio analyst with a background
in multi-asset allocation, risk management, and backtest reproducibility.

You will be given the results of a portfolio backtest structured around four
defensive umbrellas (gold, put-write, managed futures, options overlay) with
factor tilts (Quality + Momentum) on the equity core. The investor wants a
qualified opinion.

Provide your response in English, in Markdown format, structured exactly as
follows:

## 1. Strengths diagnosis
3-5 bullet points on the strengths that emerged from the backtest results.

## 2. Weaknesses diagnosis
3-5 bullet points on the weaknesses or caveats that emerged.

## 3. Parametric recommendations
3-5 concrete suggestions with specific numbers (e.g. "consider raising DBi
from 5% to 7% NAV" or "the options budget of 0.30% could be increased to 0.50%
given the robustness shown"). Each recommendation must cite the backtest figure
that motivates it.

## 4. Methodological caveats
2-3 limitations of the analysis the investor should keep in mind.

## 5. Final verdict
A 2-3 sentence synthesis summarising your overall opinion.

Be direct, numeric, and grounded in the data provided. Avoid generalities.
If a piece of data is not supplied, say so explicitly.
"""


# Max output tokens requested from every provider. Set high enough to fit the
# 5-section structured analysis required by SYSTEM_PROMPT (Strengths,
# Weaknesses, Recommendations, Caveats, Verdict). The previous value of 2000
# truncated Claude / GPT mid-section. Providers bill per actual tokens
# generated, so the ceiling costs nothing until it is reached.
...
class AiResponse:
    ...
    # True when the provider stopped because the output hit MAX_OUTPUT_TOKENS
    # (OpenAI/OpenRouter/Local: finish_reason == "length";
    #  Anthropic:                stop_reason   == "max_tokens").
    # The UI surfaces this as a warning so the user knows the analysis is cut.
...
def _openai_compatible_truncated(data: dict) -> bool:
    """`finish_reason == "length"` means the model stopped because it hit
    the request's `max_tokens` cap — i.e., the response is cut. Other values
    (`"stop"`, `"tool_calls"`, `"content_filter"`, `None`) are not truncations
    we can fix by raising `MAX_OUTPUT_TOKENS`."""
    try:
...
class OpenAiAnalyzer(AiAnalyzer):
    def analyze(self, prompt: str, system_prompt: str = SYSTEM_PROMPT) -> AiResponse:
        ...
        return AiResponse(
            ...
            truncated=_openai_compatible_truncated(data),
        )
...
def build_analysis_prompt(
    ...
    parts.extend([
        "",
        "Analyse these results following the structure required by the system prompt.",
    ])
    return "\n".join(parts)
...
def save_analysis(response: AiResponse, output_path: Path) -> None:
    ...
    if response.truncated:
        header.append(
            f"- **⚠️ Truncated**: response hit the {MAX_OUTPUT_TOKENS}-token output cap "
            "and may be incomplete. Re-run with a higher cap or a more concise prompt."
        )
    header.extend([
        "",
        "> *Note: this analysis is generated by an AI model and must be evaluated "
        "critically. It does not constitute financial advice.*",
        "",
        "---",
        "",
    ])
